...
const Header = () => {
  ...
  const toggleBalanceVisibility = () => {
    setIsBalanceVisible((prev) => !prev); // Переключаем видимость баланса
  };
  ...
  const handleOutsideClick = useCallback((event) => {
    ...
    const notificationsSidebar = document.querySelector(".notifications_sidebar");
    const accountSidebar = document.querySelector(".account_sidebar");
    const notificationModal = document.querySelector(".notification-modal");
    const fullscreenImageOverlay = document.querySelector(".fullscreen-image-overlay");
  
    // Проверяем, открыто ли модальное окно с уведомлением
    const isNotificationModalOpen = notificationModal && notificationModal.style.display !== 'none';
    ...
    try {
      ...
      if (activeDeposit) {
        ...
        if (response.status === 201) {
          ...
          navigate(`/payment/${customUrl}`, { state: { amount: amount, paymentOption: response.data } });
          handleCloseWithdraw();
