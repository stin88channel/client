...
useEffect(() => {
  ...
  const handlePaymentConfirmation = async () => {
    ...
    console.log("ID платежной опции:", paymentOption._id); // Логирование ID
    ...
    try {
      ...
      console.log("Отправляемый ID платежной опции:", paymentOption._id);
  ...
  const handlePaymentPending = async () => {
    ...
    try {
      if (!paymentOption || !paymentOption._id) {
        throw new Error("Платежная опция не определена");
      }
  
      // Отправка запроса на обновление статуса
      const response = await axios.post(
        `/api/pending-payment/${paymentOption._id}`, // Эндпоинт для обновления статуса
